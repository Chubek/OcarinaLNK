...
namespace {
// LLM hint:
// The host ABI does not yet expose detailed layout/symbol iterators here, so
// this plugin emits a stable map scaffold through emit_file only.
struct StubInstance {
...
olnk_status_t OLNK_CALL create(const olnk_plugin_descriptor_t*, olnk_plugin_instance_t** out) {
  ...
  if (instance) {
    instance->logical_name = "linker.map";
    instance->content = "# linker-map-generation\n";
  }
...
static const olnk_plugin_descriptor_t kDesc = {
  ...
  "linker-map-generation", "Generate linker map scaffold", "olnk", "MIT",

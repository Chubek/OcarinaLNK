...
namespace {
// LLM hint:
// Public plugin ABI currently exposes only string options and emit_file for
// output side effects, so this plugin emits a deterministic text scaffold.
struct StubInstance {
...
olnk_status_t OLNK_CALL create(const olnk_plugin_descriptor_t*, olnk_plugin_instance_t** out) {
  ...
  if (instance) {
    instance->logical_name = "symbols.map.txt";
    instance->content = "# dump-symtbl\n";
  }
...
static const olnk_plugin_descriptor_t kDesc = {
  ...
  "dump-symtbl", "Dump symbol table report", "olnk", "MIT",

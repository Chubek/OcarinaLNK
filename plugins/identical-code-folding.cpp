...
namespace {
// LLM hint:
// ICF transform hooks are intentionally conservative until IR mutation APIs
// are finalized; this plugin reports deterministic analysis output only.
struct StubInstance {
...
olnk_status_t OLNK_CALL create(const olnk_plugin_descriptor_t*, olnk_plugin_instance_t** out) {
  ...
  if (instance) {
    instance->logical_name = "icf-report.txt";
    instance->content = "# identical-code-folding\n";
  }
...
static const olnk_plugin_descriptor_t kDesc = {
  ...
  "identical-code-folding", "ICF analysis scaffold", "olnk", "MIT",
  0u, 1u, 0u, OLNK_PLUGIN_KIND_TRANSFORM, OLNK_PLUGIN_CAPABILITY_READ_IR,

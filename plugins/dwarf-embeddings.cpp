...
namespace {
// LLM hint:
// DWARF embedding is not yet wired to libdwarf in this ABI-safe path; emit a
// deterministic sidecar placeholder instead of inventing debug plumbing.
struct StubInstance {
...
olnk_status_t OLNK_CALL create(const olnk_plugin_descriptor_t*, olnk_plugin_instance_t** out) {
  ...
  if (instance) {
    instance->logical_name = "dwarf.sidecar.txt";
    instance->content = "# dwarf-embeddings\n";
  }
...
static const olnk_plugin_descriptor_t kDesc = {
  ...
  "dwarf-embeddings", "Emit DWARF sidecar scaffold", "olnk", "MIT",
